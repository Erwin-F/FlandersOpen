    ...
    public sealed class ColorString : ValueObject
    {
        ...
        public ColorString(string value)
        {
            if (string.IsNullOrWhiteSpace(value))
                throw new ColorArgumentException("Cannot be empty", nameof(value));

            if (!value.InColorRange())
                throw new ColorArgumentException("Needs to be between 000000 and FFFFFF", nameof(value));
        ...
        public ColorString(int red, int green, int blue)
        {
            if (!red.IsInColorRange())
                throw new ColorArgumentException("Needs to be in range 0 - 255", nameof(red));

            if (!green.IsInColorRange())
                throw new ColorArgumentException("Needs to be in range 0 - 255", nameof(green));

            if (!blue.IsInColorRange())
                throw new ColorArgumentException("Needs to be in range 0 - 255", nameof(blue));
